 ...
 *
 *  Provides a callback for "received dvalue" for a fully parsed dvalue.
 *  User code behind the callback must eventually free the dvalue using
 *  duk_dvalue_free().  Also provides a generic "cooperate" callback which
 *  is called whenever Duktape calls into any transport callbacks; you can
 *  use the cooperate callback to check for inbound events and send dvalues
 *  towards Duktape.
 *
 ...
 *      duk_trans_dvalue_send() from any thread.  When you next call into
 *      Duktape or call duk_debugger_cooperate(), the queued data will be
 *      processed.
 *
 *  Dvalues are allocated using malloc() and free(), not the Duktape heap
 *  alloc functions, to avoid disturbing the Duktape heap while debugging.
 *
 *  Not fully portable but doesn't depend on C99 types, e.g. assumes
 *  "int" is at least 32-bit signed.
 */
...
static void duk__dvalue_bufesc(duk_dvalue *dv, char *buf, size_t maxlen, int stresc) {
	size_t i, limit;

	limit = dv->len <= maxlen ? dv->len : maxlen;
	*buf = (char) 0;
	for (i = 0; i < limit; i++) {
...
void duk_dvalue_to_string(duk_dvalue *dv, char *buf) {
	...
	case DUK_DVALUE_LIGHTFUNC:
		duk__dvalue_bufesc(dv, hexbuf, 32, 0);
		sprintf(buf, "lfunc:%d:%s", dv->i, hexbuf);
...
static int duk__trans_dvalue_parse_i32(duk_trans_dvalue_ctx *ctx, unsigned char *p) {
	return (int) duk__trans_dvalue_parse_u32(ctx, p);
}
...
void duk_trans_dvalue_send(duk_trans_dvalue_ctx *ctx, duk_dvalue *dv) {
	...
	case DUK_DVALUE_NUMBER: {
		p = duk__trans_buffer_ensure(&ctx->send_buf, 1 + 8);
		if (!p) { goto alloc_error; }
		*p++ = 0x1a;
		p = duk__trans_dvalue_encode_double(ctx, p, dv->d);
		ctx->send_buf.write_offset += 1 + 8;
		break;
	}
	case DUK_DVALUE_OBJECT: {
		size_t i = dv->len;
		if (i <= 0xffUL && dv->i >= 0 && dv->i <= 0xffL) {
			p = duk__trans_buffer_ensure(&ctx->send_buf, 1 + 2 + i);
			if (!p) { goto alloc_error; }
			*p++ = 0x1b;
			*p++ = (unsigned char) dv->i;
			*p++ = (unsigned char) i;
			memcpy((void *) p, (const void *) dv->buf, i);
			ctx->send_buf.write_offset += 1 + 2 + i;
		} else {
			goto dvalue_error;
		}
		break;
	...
	case DUK_DVALUE_POINTER: {
		size_t i = dv->len;
		if (i <= 0xffUL) {
			p = duk__trans_buffer_ensure(&ctx->send_buf, 1 + 1 + i);
			if (!p) { goto alloc_error; }
			*p++ = 0x1c;
			*p++ = (unsigned char) i;
			memcpy((void *) p, (const void *) dv->buf, i);
			ctx->send_buf.write_offset += 1 + 1 + i;
		} else {
			goto dvalue_error;
		}
		break;
	...
	case DUK_DVALUE_LIGHTFUNC: {
		...
		if (i <= 0xffUL && dv->i >= 0 && dv->i <= 0xffffL) {
			p = duk__trans_buffer_ensure(&ctx->send_buf, 1 + 3 + i);
			if (!p) { goto alloc_error; }
			*p++ = 0x1d;
			p = duk__trans_dvalue_encode_u16(ctx, p, (unsigned int) dv->i);
			*p++ = (unsigned char) i;
			memcpy((void *) p, (const void *) dv->buf, i);
			ctx->send_buf.write_offset += 1 + 3 + i;
		} else {
			goto dvalue_error;
		}
		break;
...
static duk_dvalue *duk__trans_trial_parse_dvalue(duk_trans_dvalue_ctx *ctx) {
	...
	if (ib <= 0x1fU) {
		...
		case 0x10: {
			int intval;
			if (len < 1 + 4) { goto partial; }
			intval = duk__trans_dvalue_parse_i32(ctx, p + 1);
			ctx->recv_buf.read_offset += 1 + 4;
		...
		case 0x11: {
			...
			if (len < 1 + 4 + datalen) { goto partial; }
			ctx->recv_buf.read_offset += 1 + 4 + datalen;
			dv = duk_dvalue_make_tag_data(DUK_DVALUE_STRING, (const char *) (p + 5), datalen);
			if (!dv) { goto alloc_error; }
			return dv;
		}
		case 0x12: {
			if (len < 1 + 2) { goto partial; }
			datalen = (size_t) duk__trans_dvalue_parse_u16(ctx, p + 1);
			if (len < 1 + 2 + datalen) { goto partial; }
			ctx->recv_buf.read_offset += 1 + 2 + datalen;
		...
		case 0x1a: {
			double dblval;
			if (len < 1 + 8) { goto partial; }
			dblval = duk__trans_dvalue_parse_double(ctx, p + 1);
			ctx->recv_buf.read_offset += 1 + 8;
		...
		case 0x1c: {
			if (len < 1 + 1) { goto partial; }
			datalen = (size_t) p[1];
			if (len < 1 + 1 + datalen) { goto partial; }
			ctx->recv_buf.read_offset += 1 + 1 + datalen;
		...
		case 0x1d: {
			int lf_flags;
			if (len < 1 + 3) { goto partial; }
			datalen = (size_t) p[3];
			if (len < 1 + 3 + datalen) { goto partial; }
			lf_flags = (int) duk__trans_dvalue_parse_u16(ctx, p + 1);
			ctx->recv_buf.read_offset += 1 + 3 + datalen;
...
duk_size_t duk_debug_trans_dvalue_read(void *udata, char *buffer, duk_size_t length) {
...
duk_size_t duk_debug_trans_dvalue_write(void *udata, const char *buffer, duk_size_t length) {
	duk_trans_dvalue_ctx *ctx = (duk_trans_dvalue_ctx *) udata;
	unsigned char *p;
...
duk_size_t duk_debug_trans_dvalue_peek(void *udata) {
	duk_trans_dvalue_ctx *ctx = (duk_trans_dvalue_ctx *) udata;
	size_t avail;
...
void duk_debug_trans_dvalue_read_flush(void *udata) {
	duk_trans_dvalue_ctx *ctx = (duk_trans_dvalue_ctx *) udata;
...
void duk_debug_trans_dvalue_write_flush(void *udata) {
	duk_trans_dvalue_ctx *ctx = (duk_trans_dvalue_ctx *) udata;
...
void duk_debug_trans_dvalue_detached(void *udata) {
	duk_trans_dvalue_ctx *ctx = (duk_trans_dvalue_ctx *) udata;
